...
path='C:\Users\Sri Dhinesh\Documents\MATLAB\post poster works\AGORA final run model from Subshree PC\1_Dhinesh\';
files=dir(path);
models={};
for i=3:numel(files)
    models = [models;files(i).name];
end
load('C:\Users\Sri Dhinesh\Documents\MATLAB\epistasis_codes\diets.mat\')
ex_fluxes =strrep(ex_fluxes,'-','_');
ex_fluxes{13,1} = 'EX_pydx(e)';
diet_cons = ex_values(:,3); % calling only the EU avg diet

for i=1:numel(models)
    model_folder='C:\Users\Sri Dhinesh\Documents\MATLAB\post poster works\AGORA final run model from Subshree PC\1_Dhinesh\';
    ...
    save_folder=['C:\Users\Sri Dhinesh\Documents\MATLAB\post poster works\AGORA final run model from Subashree PC outputs\',models{i}];
